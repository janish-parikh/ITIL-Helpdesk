...
class Queue(models.Model):
    ...
    def prepare_permission_name(self):
        ...
        self.permission_name = "api.%s" % basename
        return basename
...
def attachment_path(instance, filename):
    ...
    import os
    os.umask(0)
    path = 'it_helpdesk/attachments/%s/%s' % (instance.followup.ticket.id, instance.followup.id)
...
class KBCategory(models.Model):
    ...
    # def get_absolute_url(self):
    #     from django.urls import reverse
    #     return reverse('api:kb_category', kwargs={'slug': self.slug})
...
def create_usersettings(sender, instance, created, **kwargs):
    ...
    """
    from api.settings import DEFAULT_USER_SETTINGS
    if created:
